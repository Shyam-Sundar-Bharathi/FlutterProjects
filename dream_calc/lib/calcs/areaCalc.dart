...
String area (String userInput, String dropDownValue, int precision) {
  if(dropDownValue == "SQUARE"){
    ...
    return (side*side).toStringAsFixed(precision);
  }
  else if(dropDownValue == "CIRCLE"){
    return (3.141592653589793238*double.parse(userInput)*double.parse(userInput)).toStringAsFixed(precision);
  }
  else if (dropDownValue == "RECTANGLE"){
    var arr = userInput.split(",");
    return (double.parse(arr[0])*double.parse(arr[1])).toStringAsFixed(precision);
  }
  else if (dropDownValue == "TRIANGLE"){
    var arr = userInput.split(",");
    if (arr.length == 2){
      return (0.5 * double.parse(arr[0]) * double.parse(arr[1])).toStringAsFixed(precision);
    }
